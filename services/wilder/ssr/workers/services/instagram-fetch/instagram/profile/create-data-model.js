...
const dataModelMentionsList = (text, emails, posts) => {
    const usernames = []
    ...
    if (posts.length !== 0) {
        posts.forEach(item => {
            item.mentionsList.forEach(mention => {
                if (usernames.indexOf(mention) === - 1) {
                    usernames.push(mention)
                }
            })
        })
...
const profileDataModel = (json) => {
    ...
    try {
        ...
        return {
            ...
            avgVideoViews,
            emailsList,
            mentionsList: dataModelMentionsList(biography, emailsList, postsList),
